module.controller("TeamEmployeeChartController", function ($scope, $http) {
    ...
    function loadData() {
        ...
        $http.jsonp(apiBaseUrl + "/Analytics/EmployeeChart?from=" + $scope.from + "&to=" + $scope.to + "&id=" + teamId + "&callback=JSON_CALLBACK").success(function (data) {
...
module.directive('employeechart', function () {
    return {
        restrict: 'A',
        scope: {
            employeedata: "=",
        ...
        link: function (scope, element, attrs) {
            ...
            //find max
            var maxY = 0;
            for (var i = 0; i < employeedata[0].data.length; i++) {
                var count = 0;
                for (var j = 0; j < employeedata.length - 1; j++) {
                    count += employeedata[j].data[i].y;
                }
            ...
            var chart = new Highcharts.Chart({
                ...
                xAxis:
                {
                    ...
                    range: 604800000, //7 days
                    minRange: 604800000 //7 days
                },
                yAxis: {
                    min: 0,
                    max: maxY,
            ...
            scope.$watch('employeedata', function (newVal, oldVal) {
