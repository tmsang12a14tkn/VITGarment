...
function drawchart(goal) {
    ...
    goal.GoalDetails.forEach(function (detail, key) {
        ...
        empData.push(detail.NoEmployee);
    })
    var currentHourIndex = timeData.indexOf(currentHour);

    $("#team_" + goal.TeamId + "_hourchart").highcharts({
        title: {
    ...
    $("#team_" + goal.TeamId + "_empchart").highcharts({
        title: {
...
app.controller("goalscontroller", function ($scope, $interval, $http, $sce) {
    var update = function () {
        ...
        .then(function (response) {
            $scope.goals = response.data.GoalModels;
